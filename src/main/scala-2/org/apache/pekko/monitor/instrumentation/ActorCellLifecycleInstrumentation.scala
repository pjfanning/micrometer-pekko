 ...
 *
 * Under Scala 2 the mixin forwarders that Pekko generates in ActorCell are ordinary methods and they are
 * what runs, so ActorCell is the type to match on. Matching the declaring trait instead would advise twice,
 * because the forwarder calls straight into the trait's default method.
 */
@Aspect
