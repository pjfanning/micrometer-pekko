...
private[pekko] case class EntityFilter(includes: List[PathFilter], excludes: List[PathFilter]) {
  def accept(name: String): Boolean =
    includes.exists(_.accept(name)) && !excludes.exists(_.accept(name))
}

private[pekko] trait PathFilter {
  def accept(path: String): Boolean
}

private[pekko] case class RegexPathFilter(path: String) extends PathFilter {
...
/**
  ...
  */
private[pekko] case class GlobPathFilter(glob: String) extends PathFilter {
